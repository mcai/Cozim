...
def main():
    Version = "Mojo v3.0 is licensed under the GPLv3 copyright 2013 Matthew O'Gorman"

    parser = argparse.ArgumentParser(description='Mojo bitstream loader v3')
...
def install_mojo(ser, bitstream, verbose, no_verify, ram):
    ...
    if ram:
        ...
        if verbose and ret == 'R':
            print 'Mojo is ready to receive bitstream'
        elif ret != 'R':
    ...
    if not ram:
        ...
        ret = ser.read(1)
        if verbose and ret == 'D':
            print 'Mojo has been loaded bitstream'
        elif ret != 'D':
